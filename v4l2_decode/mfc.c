						...
						unsigned int size, int count)
{
	...
	if (ret != 0) {
		err("Failed to Get OUTPUT(mfc.fd=%d, pixformat=0x%08x = '%c%c%c%c') "
			"for MFC decoding, ret=%d, errno=%d",
			i->mfc.fd, (unsigned int)codec,
			(char)(codec & 0xff), (char)((codec & 0x0000ff00)>>8),
	...
	if (ret != 0) {
		err("Failed to setup OUTPUT for MFC decoding, errno=%d",
			errno);
		return -1;
	}
...
int mfc_dec_queue_buf_cap(struct instance *i, int n)
{
	...
	return mfc_dec_queue_buf(i, n, i->mfc.cap_buf_size[0],
		i->mfc.cap_buf_size[0], V4L2_BUF_TYPE_VIDEO_CAPTURE,
		MFC_CAP_PLANES);
}
