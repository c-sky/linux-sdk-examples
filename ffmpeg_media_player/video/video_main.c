...
void *mfc_thread_func(void *args)
{
	...
	while (!i->error && !i->finish) {
		...
		fb_power_on(i);
		fb_wait_for_vsync(i);
		/* TODO: force to delay to process 24 or 25 frames,
		 * wait time = 1 frame time */
		usleep(i->ps->delay);
