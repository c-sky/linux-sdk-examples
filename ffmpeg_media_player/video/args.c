...
#define CSKY_MEDIA_PLAYER_MAJOR_NUM (1)
#define CSKY_MEDIA_PLAYER_MINOR_NUM (0)

void print_usage(char *name)
{
	// "d:i:m:c:V"
	// for example: MEDIA_PLAYER -c mpeg4 -d /dev/fb0 -i /mnt/nfs/mpeg4_demo.mp4 -m /dev/video3 -V
	printf("version: %d.%d\n", CSKY_MEDIA_PLAYER_MAJOR_NUM,
	       CSKY_MEDIA_PLAYER_MINOR_NUM);
	printf("Usage:\n");
	printf("\t./%s\n", name);
...
int parse_args(struct instance *i, int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "c:d:f:i:m:t:Vr:o:")) != -1) {
		switch (c) {
		case 'c':
